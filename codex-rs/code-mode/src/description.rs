...
fn mcp_structured_content_schema(output_schema: Option<&JsonValue>) -> Option<&JsonValue> {
    ...
    let Some(content_schema) = properties.get("content").and_then(JsonValue::as_object) else {
        return None;
    };
    ...
    if !properties
        .get("isError")
        .and_then(JsonValue::as_object)
        .is_some_and(|schema| schema.get("type").and_then(JsonValue::as_str) == Some("boolean"))
    {
        return None;
    }
    ...
    if !properties
        .get("_meta")
        .and_then(JsonValue::as_object)
        .is_some_and(|schema| schema.get("type").and_then(JsonValue::as_str) == Some("object"))
    {
        return None;
    }
